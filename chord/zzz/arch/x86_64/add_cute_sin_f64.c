...
void chord_zzz_arch_add_cute_sin_f64(double *restrict p, uintptr_t n, chord_aq_builder_s *restrict aq, intptr_t aq_index)
{
	...
	while (n)
	{
		...
		chord_zzz_arch_inner_vfma_f64(p, br, ba, m);
		aq_index += m;
		p += m;
		n -= m;
	}
...
dyl_export(chord_zzz_arch_add_cute_sin_f64, chord_zzz_add_cute_sin_f64);
