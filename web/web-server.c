...
static web_server_route_s* web_server_route_alloc(web_server_request_f func, refer_t pri, web_server_request_flag_t flags)
{
...
web_server_s* web_server_alloc(const uhttp_inst_s *restrict http_inst, const web_server_limit_t *restrict limit)
{
	...
	if (n && (r = (web_server_pri_s *) refer_alloz(sizeof(web_server_pri_s) + n * sizeof(yaw_s *))))
	{
		...
		for (i = 0; i < n; ++i)
		{
			if (!(working = yaw_alloc_and_start(web_server_thread_working, NULL, r)))
				goto label_fail;
		}
		return r;
		label_fail:
