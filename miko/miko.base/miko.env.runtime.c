...
static void miko_env_runtime_free_func(miko_env_runtime_s *restrict r)
{
...
{
	miko_env_runtime_s *restrict r;
	if ((r = (miko_env_runtime_s *) refer_alloz(sizeof(miko_env_runtime_s))))
	{
		refer_hook_free(r, env_runtime);
		if ((r->segment = miko_table_create_single(env->segment)) &&
			(r->action = miko_table_create_single(env->action)) &&
			(r->interrupt = miko_table_create_single(env->interrupt)) &&
			(r->major = miko_table_create_single(env->major)) &&
			(r->process = miko_table_create_single(env->process)) &&
...
{
	if (index < r->interrupt->count)
		return (refer_string_t) r->interrupt->table[index];
	return NULL;
}

refer_string_t miko_env_runtime_interrupt_by_name(const miko_env_runtime_s *restrict r, const char *restrict name)
{
	return (refer_string_t) r->interrupt->table[miko_table_index(r->interrupt, name)];
}
